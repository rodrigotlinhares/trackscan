...
void naya::Initialize3DOFxi_tempA(int size_template_x,
                                int size_template_y,
                                int n_ctrl_pts_xi,
                                int n_ctrl_pts_yi,
                                int n_active_pixels,
                                ...
                                int interp)
{
    ...
    delta.create(2+total_n_ctrl_ptsi+1, 1, CV_32FC1);
...
//        expected = (float*)malloc(n_bins*sizeof(float));
    ...
    else
    {
        ...
        SD.create(n_active_pixels*2, 2+total_n_ctrl_ptsi+1, CV_32FC1);
    ...
    for(int i=0; i<N_PROCS-1; i++)
    {
        ...
        ok_hess[i] = cvRect(0, i*part4, 2+total_n_ctrl_ptsi+1, part4);
    }
    ...
    ok_hess[N_PROCS-1] = cvRect(0, (N_PROCS-1)*part4, 2+total_n_ctrl_ptsi+1, 2*n_active_pixels - (N_PROCS-1)*part4);
    ...
    {
        pre_SD[i].create(2+total_n_ctrl_ptsi+1, 1, CV_32FC1);
        pre_hess[i].create(2+total_n_ctrl_ptsi+1, 2+total_n_ctrl_ptsi+1, CV_32FC1);
    }
}



void	naya::pWarp3DOF1_tempA(unsigned int ch)
{
...
void	naya::MountJacobian3DOFColorxi_tempA(unsigned int ch)
{
    int i1, j1, i2, j2;
    int offx = cvCeil((double)size_template_x/2);
    int offy = cvCeil((double)size_template_y/2);
...
//			SD.at<float>(k, 3+total_n_ctrl_ptsi) = 2;
            ...
            for(int i=0; i<total_n_ctrl_ptsi; i++)
                temp[i+2] = MKinvi.at<float>(active_pixels_g[k], i)*(current_pxl + current_tmplt);

            temp[2+total_n_ctrl_ptsi] = 2;

            for(int i=0;i<=(total_n_ctrl_ptsi+2);i++)
            {
                for(int j=i;j<=(total_n_ctrl_ptsi+2);j++)
                {
...
//			SD.at<float>(k+ n_active_pixels, 3+total_n_ctrl_ptsi) = 2;
            ...
            for(int i=0; i<total_n_ctrl_ptsi; i++)
                temp[i+2] = MKinvi.at<float>(active_pixels_r[k], i)*(current_pxl + current_tmplt);
...
int		naya::Update3DOFix_tempA()
{
    ...
    // Update
    parameters[2] -= delta.at<float>(0, 0); sum += fabs(delta.at<float>(0, 0));
    parameters[3] -= delta.at<float>(1, 0); sum += fabs(delta.at<float>(1, 0));

    for(int i=0; i<total_n_ctrl_ptsi; i++)
    {
        ctrl_pts_wi.at<float>(i, 0) -= delta.at<float>(2+i, 0);
        sum += fabs(delta.at<float>(2+i, 0));
    }

    bias -= delta.at<float>(total_n_ctrl_ptsi+2, 0);
...
void 	naya::Run3DOFxi_threaded_tempA(cv::Mat *ICur,
                            cv::Mat *Mask_roi,
                            cv::Mat *Template,
                           ...
                           int *active_pixels_g)
{
    ...
    for(int i=0; i<n_max_iters; i++)
    {
        ...
        if(!flag_done)
        {
            ...
            #pragma omp barrier
            MountJacobian3DOFColorxi_tempA(procInfo);
